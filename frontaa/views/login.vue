    ...
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
      ...
      <el-form-item prop="code">
        <el-input
          ...
          style="width: 240px; margin-right: 5px; margin-top:5px; "
        ></el-input>
      ...
      <el-form-item prop="logintype">
        ...
        <el-radio v-model="ruleForm.logintype" label="2">业主</el-radio>
      </el-form-item>
...
import { testLink } from '../api/userMG'
...
export default {
  ...
  data() {
    return {
      //定义验证码
      captchaUrl: "http://localhost:8383/captcha?time=" + new Date(),
      //定义loading默认为false
      logining: false,
      // 记住密码 lse,
      ruleForm: {
      ...
      rules: {
        ...
        logintype:[{required: true, message: '请选择登录类别', trigger: 'blur' }],
      }
    }
  },
  // 创建完毕状态(里面是操作)
  ...
  methods: {
    updateCaptcha() {
      this.captchaUrl = "http://localhost:8383/captcha?time=" + new Date();
    },
    // 获取用户名密码
    getuserpwd() {
      if (getCookie('user') !=null && getCookie('pwd') != null) {
    ...
    submitForm() {
      this.$refs.ruleForm.validate( async valid => {
        ...
        const { data: res,meta:st } = await login(this.ruleForm);
        console.log(res)
        console.log(st)
        console.log(JSON.stringify(res))
        // if (res.meta.status !== 200)  return this.$message.error('登录失败')
        if(res==null) return this.$message.error(st.msg)
        this.$message.success('登录成功')
        setCookie('user',this.ruleForm.account);
        // console.log(this.ruleForm)
        setCookie('pwd',this.ruleForm.password);
        setCookie('permission',res.permission)
        // console.log(res)
        window.sessionStorage.setItem('token', '123')
        window.sessionStorage.setItem('logintype',res.logintype)
        window.localStorage.setItem('userdata',JSON.stringify(res))
        this.$store.commit('login', 'true');
        if(res.logintype==1){
          this.$router.push('/system/user');
        }else{
          this.$router.push('/system/Owneruser');
        }
      })
    },
  }
